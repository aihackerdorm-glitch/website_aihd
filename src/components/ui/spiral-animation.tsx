...
class AnimationController {
  ...
  public getChangeEventTime() { return this.changeEventTime }
  public getCameraTravelDistance() { return this.cameraTravelDistance }
  ...
  private setupRandomGenerator() {
    ...
    // Temporarily override Math.random to get deterministic star generation
    // Then restore the original implementation
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ;(Math as any).random = customRandom()
    this.createStars()
...
export function SpiralDemo() {
  ...
  return (
    <div className="fixed inset-0 w-full h-full overflow-hidden bg-black">
      ...
      <div
        className={`absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-10 transition-all duration-[1500ms] ease-out ${startVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}
      >
        <button
